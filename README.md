...
# 🍕 Table Clearance Detection System

> Прототип системы детекции уборки столиков в пиццерии по видеозаписи с IP-камеры.  
> Стек: **Python 3.10 · YOLOv8n · OpenCV · Pandas · Matplotlib**
...
- **Файл:** `video1.mp4`
- **Столик:** зона `x=120 y=80 w=240 h=160` — левый ближний столик у окна
...
| Метрика                                          | Значение  |
|--------------------------------------------------|-----------|
| Завершённых циклов (OCCUPIED → EMPTY → APPROACH) | **4**     |
| Среднее время пустого стола между гостями        | **34.2 с**|
| Медиана                                          | **28.5 с**|
| Минимум / Максимум                               | **12.1 / 67.8 с**|

> Результаты зависят от конкретного видео. Для получения точных цифр запустите систему  
> с `--report` и проверьте файл `report.txt` в папке сессии.
